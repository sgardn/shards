  ...
  chrome.tabs.executeScript(tab.id, {
    code: `
      ...
      if (!node) {
        ...
        var text = document.createTextNode("Partner has played:"); // text node isn't really an HTML node, just text inside the element
        node.appendChild(text);
        node.style = "background-color:blue; position: relative; top: 60%; display: flex; flex-direction: row; align-items: center; justify-content: center; color: white;"
        document.querySelector(".dealViewerToolbarClass").appendChild(node);
        inner = document.createElement("DIV");
        inner.setAttribute("id", "splinter-contents")
      ...
      let handle = setInterval(function() {
        ...
        let suitSignals = getPartnerSignals(cardsPlayedByPartner, cardsLed)
        // write to DOM based on results
        contents = document.querySelector("#splinter-contents")
        contents.innerHTML = suitSignals[0].join(" ") + suitSignals[1].join(" ") + suitSignals[2].join(" ") + suitSignals[3].join(" ")
      }, 500);
